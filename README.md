...
Repository kini memiliki fondasi Source Scaffolding yang lengkap dan tervalidasi, dan IVS-01 sampai IVS-07 sudah COMPLETE. Backend Java 21/Spring Boot/Maven dengan PostgreSQL/Flyway/JdbcClient kini menyediakan OIDC provider-neutral, sesi browser Teacher yang backend-managed, `GET /api/teacher-session`, `POST /api/classroom-sessions`, PairingGrant lima menit yang aman dan single-use, participant session Controller/Display dengan lifetime absolut delapan jam secara fixed/non-sliding tanpa idle timeout, rekonsiliasi Revision Controller, command `DIRECT_ACTION:NEXT` yang transactional dan idempotent, serta snapshot Display berotoritas participant yang diturunkan dari ClassroomSession dan LessonVersion immutable dalam schema `1.0` `PLAIN_TEXT`. Snapshot tidak menyinkronkan Display; production Display mutation gate tetap fail-closed sampai IVS-08 menyediakan bukti sinkronisasi nyata. OIQ-04 kini RESOLVED melalui baseline heartbeat/liveness SSE di implementation plan, sehingga IVS-08 READY TO EXECUTE tetapi runtime-nya belum dimulai. Shell Teacher dan Classroom Display React/TypeScript/Vite tetap terpisah. Application Implementation tetap ACTIVE. Production IdP, deployment configuration, dan CI/CD belum dipilih atau diimplementasikan.
